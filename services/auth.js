...
const loginUser = async (username,password,platform,roleAccess) => {
  try {
    ...
    if (user) {
      const userToken = await dbService.count(userTokens, 
        { $and:[
          { tokenExpiredTime: { $gt: dayjs().toISOString() }, },
          { isTokenExpired: 0, },
          { userId:user.id }
        ]}
      );
      ...
      else if (platform == PLATFORM.SYSTEM){
        if (!LOGIN_ACCESS[user.userType].includes(PLATFORM.SYSTEM)){
          return {
            flag:true,
            data:'you are unable to access this platform'
          };
        }
        token = await generateToken(userData,JWT.SYSTEM_SECRET);
      }
