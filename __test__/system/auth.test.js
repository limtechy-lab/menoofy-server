...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Immanuel.Beahan',
        'password':'vMnHCtun066dQe7',
        'email':'George.Gutkowski@gmail.com',
        'name':'Jesus Bosco',
        'userType':authConstant.USER_TYPES.System,
        'mobileNo':'(839) 224-0094',
        'ssoAuth':{ 'googleId':'Administrator' }
      });
...
describe('POST /login -> if username and password is correct', () => {
  test('should return user with authentication token', async () => {
    let user = await request(app)
      ...
      .send(
        {
          username: 'George.Gutkowski@gmail.com',
          password: 'vMnHCtun066dQe7'
        }
      );
...
describe('POST /forgot-password -> if email has not passed from request body', () => {
  test('should return bad request status and insufficient parameters', async () => {
    let user = await request(app)
      .post('/system/auth/forgot-password')
      .send({ email: '' });
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    let user = await request(app)
      .post('/system/auth/forgot-password')
      .send({ 'email':'George.Gutkowski@gmail.com', });
  ...
  test('should return success', () => {
    return request(app)
      ...
      .send(
        ...
        return request(app)
          .get(`/system/api/v1/user/${login.body.data.id}`)
          .set({
            Accept: 'application/json',
            Authorization: `Bearer ${login.body.data.token}`
          }).then(foundUser => {
            return request(app)
              .post('/system/auth/validate-otp')
...
describe('PUT /reset-password -> if request body is empty or code/newPassword is not given', () => {
  test('should return insufficient parameter', async () => {
    let user = await request(app)
      .put('/system/auth/reset-password')
      .send({});
