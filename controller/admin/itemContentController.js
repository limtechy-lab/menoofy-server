...
/**
 * @description : create document of ItemContent in mongodb collection.
 * @param {Object} req : request including body for creating document.
 * @param {Object} res : response of created document
 * @return {Object} : created ItemContent. {status, message, data}
 */ 
const addItemContent = async (req, res) => {
  try {
    let dataToCreate = { ...req.body || {} };
    let validateRequest = validation.validateParamsWithJoi(
      dataToCreate,
      itemContentSchemaKey.schemaKeys);
    if (!validateRequest.isValid) {
      return res.validationError({ message : `Invalid values in parameters, ${validateRequest.message}` });
    }
    dataToCreate.addedBy = req.user.id;
    dataToCreate = new ItemContent(dataToCreate);
    let createdItemContent = await dbService.create(ItemContent,dataToCreate);
    return res.success({ data : createdItemContent });
  } catch (error) {
...
/**
 * @description : create multiple documents of ItemContent in mongodb collection.
 * @param {Object} req : request including body for creating documents.
 * @param {Object} res : response of created documents.
 * @return {Object} : created ItemContents. {status, message, data}
 */
const bulkInsertItemContent = async (req,res)=>{
  try {
    ...
    let createdItemContents = await dbService.create(ItemContent,dataToCreate);
    createdItemContents = { count: createdItemContents ? createdItemContents.length : 0 };
    return res.success({ data:{ count:createdItemContents.count || 0 } });
  } catch (error){
    return res.internalServerError({ message:error.message });
  }
};
...
const deleteItemContent = async (req,res) => {
  try {
    ...
    let deletedItemContent;
    if (req.body.isWarning) { 
      deletedItemContent = await deleteDependentService.countItemContent(query);
    } else {
      deletedItemContent = await deleteDependentService.deleteItemContent(query);
    }
